...
public class Vector {
    ...
    public double dotProduct(Vector other) {
        return IntStream.range(0, other.getDimensions())
    ...
    public Vector matProduct(Matrix m) {
        //assertCorrectDimensions(m.rows);
        double[][] matrix = m.getData();
        double[] v = new double[m.cols];

        System.out.println("Data: " + data.length);
        

        //System.out.println("Vector: " + this.getDimensions());
        System.out.println("Matrix: " + m.rows + ", " + m.cols);

        for (int col = 0; col < m.cols; col++) {
            for (int row = 0; row < m.rows; row++) {
                v[col] += m.getData()[row][col] * data[row];
            }
        }
    ...
    public Matrix outerProdut(Vector other) {
        ...
        double[][] d = new double[other.getDimensions()][this.getDimensions()];
    ...
    private void assertCorrectDimensions(int dimensions) {
        if (this.getDimensions() != dimensions) {
            throw new IllegalArgumentException("Different dimensions: Input is %d, Vec is %d");
        }
    }
